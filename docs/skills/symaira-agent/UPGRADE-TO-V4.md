...
symaira auth status --output json
symaira agent list --output json   # may not exist on pre-v4 binaries; tolerate failure
```
...
(`symaira mcp install`, `symaira mcp-config`, `symaira mcp token …`,
`symaira mcp-token-rotate`, `symaira agent setup`). These will need to be
...
```bash
symaira backup ~/symaira-backup-pre-v4-$(date +%Y%m%d-%H%M%S).tar.gz
```

If `symaira backup` is unavailable, fall back to:

```bash
cp -a ~/.symaira ~/.symaira.backup.pre-v4.$(date +%Y%m%d-%H%M%S)
```
...
|---|---|
| Homebrew (macOS, Linux) | `brew upgrade symaira` |
...
| Scoop (Windows) | `scoop update symaira` |
| Nix | `nix profile upgrade symaira` (or rebuild the flake input) |
...
1. Back up `~/.symaira/config.yaml` to `config.yaml.bak.v3-<timestamp>`.
...
(`cp ~/.symaira/config.yaml.bak.v3-<timestamp> ~/.symaira/config.yaml`) and
...
symaira agent doctor --all
```

For each agent that comes back with warnings or errors:

- **Skill drift** — re-render in place:
  `symaira agent install <name> --skill-only --force`
- **Missing MCP config entry** — re-inject:
  `symaira agent install <name> --config-only`
- **Token invalid / expired** — rotate:
  `symaira agent token <name> rotate`
- **Profile classified `custom` but user wants a preset** — explicit upgrade:
  `symaira agent upgrade <name> --tier <safe|standard|admin>` (interactive)

Run `symaira agent whoami --output json` (or
`OPENPASS_AGENT=<name> symaira agent whoami --output json`) to confirm the
...
|---|---|
| `symaira agent setup <name>` | `symaira agent install <name>` |
| `symaira mcp install <name>` | `symaira agent install <name>` |
| `symaira mcp install --auto-detect` | `symaira agent install --auto-detect` |
| `symaira mcp-config <agent>` | `symaira agent install <agent> --config-only` |
| `symaira mcp token create …` | `symaira agent token <name> new …` |
| `symaira mcp token list` | `symaira agent token list` |
| `symaira mcp token revoke <id>` | `symaira agent token revoke <id>` |
| `symaira mcp-token-rotate` | `symaira agent token <name> rotate` |
...
```bash
# Before
symaira agent setup hermes --tier admin

# After
symaira agent upgrade hermes --tier admin --yes --reason "<ticket-or-context>"
```
...
Then the agent can call `symaira list --output json`,
`symaira get path/to/entry --metadata-only --output json`, etc., and Symaira Vault
...
- Audit-log location: `~/.symaira/audit/`
...
```bash
symaira agent audit self --since 7d --output json
```
...
- Do not write profile fields you do not understand. If `symaira migrate v4`
...
1. Stop any running Symaira Vault processes: `pkill symaira` (best-effort).
...
3. Restore config:
   ```bash
   cp ~/.symaira/config.yaml.bak.v3-<timestamp> ~/.symaira/config.yaml
   ```
4. If skills were written to agent dirs and the user wants them gone, list
   them with `find ~ -path '*/skills/symaira/SKILL.md'` and remove only the
   ones with `managed_by: symaira` in their frontmatter.
5. Tokens issued by v4 remain valid under v3 but v3 ignores the new metadata
   fields. The user can rotate them with `symaira mcp-token-rotate` on v3.
