...
|| 2026-08-08 | Automated re-audit | DEFER | Re-audit for #739: `go.mod` unchanged since last audit — still `golang/groupcache v0.0.0-20241129210726-2c02b8208cf8` and `google.golang.org/grpc v1.82.1`, both still requiring `github.com/golang/protobuf v1.5.4`. `golang/groupcache#150` remains open, last updated 2024-04-25, no maintainer engagement. Latest go-git v5 release (v5.19.2) still depends on groupcache with the deprecated protobuf shim. **New signal:** go-git's in-development v6 line (`v6.0.0-alpha.5`, module `github.com/go-git/go-git/v6`) has dropped `groupcache` entirely from `go.mod` — the dependency chain is gone on v6. v6 is still alpha and a breaking major-version migration, so not actionable now, but this is the first concrete upstream removal signal since this deferral began; worth tracking for GA. grpc-go (`v1.84.0-dev`) still requires `golang/protobuf v1.5.4`. Status quo maintained. |
| 2026-08-20 | Automated re-audit | DEFER | Re-audit for v0.15.4 release: PR #827 updated `google.golang.org/grpc` v1.82.1→v1.83.0 and `github.com/go-git/go-git/v5` v5.19.1→v5.19.2. Both still transitively require `github.com/golang/protobuf v1.5.4` (grpc go.mod still directly requires it; groupcache still at v0.0.0-20241129 with no new commits). `golang/groupcache#150` still open, no maintainer engagement. Module graph confirms 3 edges: (1) groupcache→golang/protobuf v1.5.4, (2) grpc→golang/protobuf v1.5.4, (3) google.golang.org/protobuf→golang/protobuf v1.5.0 (intentional compat shim). No viable direct removal path; upstream migration unchanged. go-git v6 alpha still alpha (no GA). grpc-go v1.83.0 release notes show no protobuf migration. go.mod comments still reference old "OpenPass" naming — update recommended for next release cycle but no functional impact. Status quo maintained. |
| 2026-08-22 | Issue-sweep re-audit (#796) | DEFER — issue resolved as documented status quo | Re-audit for #796, verified against the live module proxy and upstream master on 2026-08-22: `go list -m -versions github.com/golang/protobuf` confirms **v1.5.4 is the latest and final published version** of the shim module — there is no newer version to update to. The groupcache `master` branch `go.mod` (fetched live) still directly requires `github.com/golang/protobuf v1.5.4` and has had no commits since 2024-11-29 (`2c02b8208cf8`, exactly the pinned pseudo-version); migration issue #150 remains open without maintainer engagement. Local module graph confirms all three edges persist under grpc v1.83.0 / go-git v5.19.2. No upstream migration occurred; no direct dependency update can resolve this. Status quo per the Decision section remains the correct resolution, now formally recorded against #796. |
| 2026-08-24 | Issue-sweep re-audit (#849) | DEFER | Re-audit for #849, verified live on 2026-08-24: groupcache upstream `go.mod` (fetched from `golang/groupcache` master) still directly requires `github.com/golang/protobuf v1.5.4`; migration issue #150 remains open without maintainer engagement. Local module graph confirms both inbound edges persist under groupcache `v0.0.0-20241129210726-2c02b8208cf8` and grpc `v1.83.0`. A direct dependency update cannot resolve this — `golang/protobuf` v1.5.4 is the final release of the shim module, and it is transitive-only (no direct imports). Additionally verified via `go list -deps ./...`: `github.com/golang/protobuf` packages are **not compiled into any Symaira Vault binary**; the deprecated module exists only in the module graph and `go.sum`, so exposure is limited to audit-tooling noise. Status quo per the Decision section maintained. |
