...
| CFG-003 | Config bytes | canonical/unknown/corrupt YAML | `internal/config.Load` / `(*Config).SaveTo` | acceptance of canonical, unknown and corrupt input; the resolved config for accepted input; the writer's canonical output reloading to the same snapshot; and the 0700/0600 modes the writer leaves behind. Error text is deliberately excluded — yaml.v3 and serde_yaml_ng word failures differently — as is `vaultDir`, which is environment-derived and belongs to CFG-001 | 20 Go-generated cases in `testdata/port/config/bytes.json`, consumed by `crates/symvault-core/tests/config_bytes_contract.rs`; provenance verified against git blobs by `configbytesgen`; wired into `port-contract` as `cfg-bytes-fixtures-check` | all; mode assertions unix-only, recorded as such rather than asserted on Windows | bytes + semantic | in_progress: green natively on ubuntu/macOS/Windows at `c67c2306` (run 34980939389, 24 jobs). **Four acceptance divergences are pinned, not resolved**, and block `PASS`: Go accepts and Rust rejects `multiple_documents` (Go silently drops every document after the first), `wrong_scalar_type` (`canWrite: "yes"` silently ignored), `negative_duration` (silently falls back to the default) and `null_document`. In three of the four Go's leniency silently discards security-relevant configuration, so the pinned oracle is not the safer side; resolving it changes which existing config files load and is a contract decision. The test asserts the divergence set exactly, so it can neither grow nor shrink unnoticed. Decision paper: `cfg-003-acceptance-adjudication.md` |
