...
| STORE-001 | vault layout | fresh and legacy vault trees | Go init/open | paths, files, modes, migration and no data loss | filesystem manifest | all | metadata + hashes | in_progress |
| STORE-002 | entry model | minimal/full/nested/large values | Go loader | YAML shape, metadata, ordering, limits, type inference | fixture suite | all/iOS | bytes + semantic | in_progress |
