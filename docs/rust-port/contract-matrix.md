...
| SESSION-002 | OS keyring | memory backend + native smoke | Go session | service/account names, binary payload, unavailable behavior | injected tests plus explicit `native_keyring_binary_roundtrip_and_delete`: macOS arm64 passed a generated test-only service/account through missing/set/binary get/update/delete/idempotent delete; no pre-existing keychain item or user vault data was read or changed. Private HOME does not isolate Apple's Keychain Services, so this remains native macOS evidence rather than isolated-keychain evidence; Linux/Windows native backends remain unverified | macOS native diagnostic; isolated and other native OS pending | semantic | in_progress |
