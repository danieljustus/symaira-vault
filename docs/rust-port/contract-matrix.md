...
>
> RUST-007 local executable evidence: `make config-session-differential` passes: the Go generators produce and check provenance-bound config, session, persistent-quota, and platform fixtures, and Rust consumes them with config/session parity, public quota-wrapper, cross-process lock, malformed/I/O, and fail-closed native-boundary tests. The generated platform fixture explicitly records injected-only native evidence. This does not promote CFG/SESSION/PLATFORM/QUOTA-002 to `PASS`: native OS keyring/UI/daemon/clipboard/autotype evidence, Windows `LockFileEx`, and CI fixture integration remain required.
>
