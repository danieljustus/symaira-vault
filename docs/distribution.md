...
curl -LO https://github.com/danieljustus/symaira-vault/releases/latest/download/openpass_<version>_linux_<arch>.deb
...
curl -LO https://github.com/danieljustus/symaira-vault/releases/latest/download/openpass_<version>_linux_<arch>.rpm
...
curl -LO https://github.com/danieljustus/symaira-vault/releases/latest/download/openpass_<version>_linux_<arch>.apk
...
sudo mv OpenPass_<version>_<os>_<arch>/symvault /usr/local/bin/

# Windows (PowerShell)
Expand-Archive OpenPass_<version>_windows_<arch>.zip
Move-Item OpenPass_<version>_windows_<arch>\symvault.exe C:\Windows\System32\
```
...
curl -LO https://github.com/danieljustus/symaira-vault/releases/latest/download/OpenPass_<version>_checksums.txt
...
TMPDIR=$(mktemp -d)
OPENPASS_VAULT="$TMPDIR/test-vault" symvault init --non-interactive
rm -rf "$TMPDIR"
```
