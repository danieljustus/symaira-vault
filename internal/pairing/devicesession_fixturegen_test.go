...
func TestDeviceSessionFixture(t *testing.T) {
	...
	if !bytes.Equal(got, content) { t.Fatal("device session fixture is stale; run UPDATE_DEVICE_SESSION_FIXTURE=1 go test ./internal/pairing -run '^TestDeviceSessionFixture$' -count=1") }
}

func buildDeviceSessionFixture(t *testing.T, root string) []byte {
	t.Helper()
	if runtime.Version() != "go1.26.6" { t.Fatalf("fixture requires pinned Go 1.26.6, got %s", runtime.Version()) }
	...
	for _, name := range deviceSessionOracleFiles {
		...
		if err != nil { t.Fatalf("read pinned source %s: %v", name, err) }
		working, err := os.ReadFile(filepath.Join(root, filepath.FromSlash(name)))
		if err != nil { t.Fatal(err) }
		if !bytes.Equal(pinned, working) { t.Fatalf("oracle source %s differs from pinned commit %s", name, deviceSessionOracleCommit) }
	...
	cases := []struct { name, token, device string; revoked bool; expires time.Time; legacy bool; cleanup bool }{
	...
	for _, input := range cases {
		...
		if input.legacy { value["token"] = input.token; key = input.token }
		...
		if input.cleanup { store.CleanupExpired() }
