...
// deviceSessionGoVersion is the toolchain the committed fixture was produced
// with. The fixture records `go_version`, so only that exact toolchain can
// verify it byte for byte: CI and every Makefile target pin GOTOOLCHAIN, and a
// plain `go test ./...` under a newer local Go skips instead of failing.
const deviceSessionGoVersion = "go1.26.6"
...
func TestDeviceSessionFixture(t *testing.T) {
	...
	generating := os.Getenv("UPDATE_DEVICE_SESSION_FIXTURE") == "1"
	if runtime.Version() != deviceSessionGoVersion && !generating {
		t.Skipf("device session fixture check needs pinned %s, got %s; run make device-session-fixtures-check", deviceSessionGoVersion, runtime.Version())
	}
...
func buildDeviceSessionFixture(t *testing.T, root string) []byte {
	t.Helper()
	if runtime.Version() != deviceSessionGoVersion {
		t.Fatalf("fixture requires pinned %s, got %s", deviceSessionGoVersion, runtime.Version())
	}
