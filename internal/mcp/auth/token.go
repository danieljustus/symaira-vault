...
// tool-registry-drift detection on scoped tokens. It is a SHA-256 digest of
// the canonical tool definitions covering name, description, and input schema.
var CurrentToolRegistryHash string
...
func (t *ScopedToken) IsToolRegistryDriftDetected() bool {
	...
	return t.ToolRegistryHash != CurrentToolRegistryHash
}
...
func (r *TokenRegistry) Create(label string, allowedTools []string, agentName string, ttl time.Duration) (*ScopedToken, string, error) {
	...
	t := &ScopedToken{
		...
		ToolRegistryHash: CurrentToolRegistryHash,
		AgentName:        agentName,
		CreatedAt:        createdAt,
		ExpiresAt:        expiresAt,
	}
	...
	if readErr == nil && strings.TrimSpace(string(legacyData)) != "" {
		...
		if _, exists := reg.Get(hash); !exists {
			...
			if saveErr := reg.Save(); saveErr == nil {
				...
				// `symaira mcp token create` with an explicit allow-list and
				// then revoke the legacy entry.
				fmt.Fprintf(os.Stderr,
					"WARNING: legacy MCP token migrated to scoped registry with wildcard (*) tool access (id=%s).\n"+
						"         To restrict scope, run: symaira mcp token create --label <name> --tools <list>\n"+
						"         Then revoke the legacy token: symaira mcp token revoke %s\n",
					id, id)
