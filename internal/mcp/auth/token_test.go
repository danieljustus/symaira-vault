...
func TestCurrentToolRegistryHash(t *testing.T) {
	// CurrentToolRegistryHash is empty by default; server init sets it at startup.
	if CurrentToolRegistryHash != "" {
		t.Error("Expected empty hash by default in auth package")
	}
	// Verify setting it works for drift detection
	CurrentToolRegistryHash = "test-hash"
	defer func() { CurrentToolRegistryHash = "" }()
	if CurrentToolRegistryHash != "test-hash" {
		t.Error("CurrentToolRegistryHash should be settable")
	}
}
...
func TestTokenRegistryHashStoredOnCreate(t *testing.T) {
	...
	if tok.ToolRegistryHash != "test-hash" {
		t.Errorf("hash mismatch: token=%q, want %q", tok.ToolRegistryHash, "test-hash")
	}
}
...
func TestTokenRegistryHashStoredOnCreateWithRefresh(t *testing.T) {
	...
	if tok.ToolRegistryHash != "test-hash" {
		t.Errorf("hash not set on CreateWithRefresh token: got %q, want %q", tok.ToolRegistryHash, "test-hash")
	}
}
