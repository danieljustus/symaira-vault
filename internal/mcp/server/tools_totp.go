...
func (s *Server) handleGenerateTOTP(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	default:
		return mcp.NewToolResultError(fmt.Sprintf("invalid destination %q: must be clipboard, autotype, or return", destination)), nil
	}
}

func (s *Server) totpClipboard(ctx context.Context, path string, code *crypto.TOTPCode) (*mcp.CallToolResult, error) {
...
func (s *Server) totpAutotype(ctx context.Context, path string, code *crypto.TOTPCode) (*mcp.CallToolResult, error) {
	if !s.canUseAutotype() {
		s.logAudit(ctx, totpToolName, path, false)
...
func (s *Server) totpReturn(ctx context.Context, path string, code *crypto.TOTPCode, returnCode bool) (*mcp.CallToolResult, error) {
	if !returnCode {
		return mcp.NewToolResultError("return_code must be true when destination is \"return\""), nil
	}
