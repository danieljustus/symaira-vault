...
func (s *Server) handleWhoami(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	_, _ = ctx, req
	...
	for _, def := range toolDefinitions() {
		...
		if def.Available != nil && !def.Available(s) {
			unavailable = append(unavailable, whoamiUnavailable{
				Name:   def.Name,
				Code:   "not_available",
				Reason: "mcp.Tool is not available in the current environment",
			})
			continue
		}
