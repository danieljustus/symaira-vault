...
func (s *Server) handleSecretUnseal(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if !ok {
		...
		return mcp.NewToolResultError(fmt.Sprintf("invalid handle format: %s", handleStr)), nil
	}
	...
	if vaultErr != nil {
		...
		return mcp.NewToolResultError(fmt.Sprintf("entry not found: %s", handle.Path)), nil
	}
	...
	if handle.Field == "" {
		...
		if !found {
			s.logAudit(ctx, "secret_unseal", entryPath, false)
			return mcp.NewToolResultError(fmt.Sprintf("field %q not found in entry %s", handle.Field, handle.Path)), nil
		}
