...
func TestProtocolHandler_ToolsList_NotInitialized(t *testing.T) {
	...
	if resp.Error.Code != transport.ErrCodeServerError {
		t.Errorf("Error code = %d, want %d", resp.Error.Code, transport.ErrCodeServerError)
	}
}
...
func TestProtocolHandler_ToolsCall_NilTools(t *testing.T) {
	...
	if resp.Error.Code != transport.ErrCodeInternalError {
		t.Errorf("Error code = %d, want %d", resp.Error.Code, transport.ErrCodeInternalError)
	}
}
