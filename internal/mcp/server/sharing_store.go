...
type shareStoreFile struct {
	...
	Grants  []mcp.ShareGrant `json:"grants"`
}
...
type ShareStore struct {
	path       string
	mu         sync.RWMutex
	grants     map[string]*mcp.ShareGrant // keyed by grant ID
	version    int
	signingKey []byte
	stopFn     func()
}
...
func NewShareStore(path string) *ShareStore {
	return &ShareStore{
		path:    path,
		grants:  make(map[string]*mcp.ShareGrant),
...
func (s *ShareStore) Load() error {
	...
	s.grants = make(map[string]*mcp.ShareGrant, len(file.Grants))
...
func (s *ShareStore) Save() error {
	...
	file := shareStoreFile{
		Version: shareStoreVersion,
		Grants:  make([]mcp.ShareGrant, 0, len(s.grants)),
	}
...
func (s *ShareStore) Create(fromAgent, toAgent, secretPath, secretField string, ttl time.Duration) (*mcp.ShareGrant, error) {
...
func (s *ShareStore) Get(id string) (*mcp.ShareGrant, bool) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *ShareStore) Revoke(id string) error {
	...
	if g.Status == mcp.ShareRevoked || g.Status == mcp.ShareRejected {
		s.mu.Unlock()
		return fmt.Errorf("share grant %s cannot be revoked (status: %s)", id, g.Status)
	}

	now := time.Now().UTC()
	g.Status = mcp.ShareRevoked
	g.RevokedAt = &now
	s.mu.Unlock()
...
// mcp.ShareFilter. When no filter is supplied, all grants are returned.
func (s *ShareStore) List(filters ...mcp.ShareFilter) []*mcp.ShareGrant {
	s.mu.RLock()
	result := make([]*mcp.ShareGrant, 0, len(s.grants))
	for _, g := range s.grants {
	...
	filter := filters[0]
	filtered := make([]*mcp.ShareGrant, 0, len(result))
...
func (s *ShareStore) ListForAgent(agentName string) []*mcp.ShareGrant {
	s.mu.RLock()
	defer s.mu.RUnlock()

	result := make([]*mcp.ShareGrant, 0)
	for _, g := range s.grants {
...
func (s *ShareStore) CheckAccess(toAgent, secretPath string) (*mcp.ShareGrant, bool) {
	s.mu.RLock()
	defer s.mu.RUnlock()
