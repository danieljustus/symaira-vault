...
// rejected as a security measure. Returns the mcp.ShareGrant and true if access
// is granted, or nil and false otherwise.
func (s *Server) checkShareAccess(ctx context.Context, path string) (*mcp.ShareGrant, bool) {
	if s.shareStore == nil {
		return nil, false
	}
...
func (s *Server) logAuditShare(ctx context.Context, action, path string, grant *mcp.ShareGrant, ok bool) {
	if s == nil || s.auditLog == nil {
		return
	}
	entry := audit.LogEntry{
