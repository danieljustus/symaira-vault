...
// SniffAndClearEnvPassphrase reads SYMVAULT_PASSPHRASE from the environment,
// stores the value in a process-local buffer, and immediately unsets the
// variable so child processes cannot inherit the raw passphrase.
//
...
func SniffAndClearEnvPassphrase() {
	cachedEnvPassphraseOnce.Do(func() {
		p := os.Getenv("SYMVAULT_PASSPHRASE")
		if p != "" {
