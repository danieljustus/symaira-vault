...
func TestHMACTamperedEntryDetected(t *testing.T) {
	...
	logFile := filepath.Join(home, configpkg.DefaultVaultSubdir, "audit-hmac-tamper-test.log")
...
func TestHMACMultipleWrites(t *testing.T) {
	...
	content, err := os.ReadFile(filepath.Join(home, configpkg.DefaultVaultSubdir, "audit-hmac-multi-test.log"))
	if err != nil {
		t.Fatalf("ReadFile error = %v", err)
	}
