...
func NewGetCmd(ops vaultpkg.OperationService) *cobra.Command {
  ...
  symvault get github.password --profile work`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return cli.WithVault(func(v *vaultpkg.Vault) error {
				...
				cli.MaybeAutoPull(s.VaultDir(), v.Config)
				query := args[0]
				path := query
				field := ""
				...
				if idx := strings.LastIndex(query, "."); idx > 0 {
					...
					if _, readErr := s.GetField(candidatePath, candidateField); readErr == nil {
						path = candidatePath
						field = candidateField
					}
				}

				value, err := s.GetField(path, field)
				if err != nil {
					...
					matches, findErr := s.FindEntries(path, vaultpkg.FindOptions{})
					if findErr != nil {
