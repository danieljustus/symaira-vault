...
func NewImportCmd(ops vaultpkg.OperationService) *cobra.Command {
  ...
  symvault import csv data.csv --overwrite`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return cli.WithVault(func(v *vaultpkg.Vault) error {
				...
				for _, entry := range entries {
					...
					exists, err := importEntryExists(s, entryPath)
					if err != nil {
					...
					if exists && options.Overwrite {
						if err := s.DeleteEntry(entryPath); err != nil {
							return fmt.Errorf("cannot overwrite entry: %w", err)
						}
					}

					record := vaultpkg.WriteRecord{Action: "import"}
					if err := s.UpsertEntry(entryPath, entry.Data, "set", &record); err != nil {
	...
	importReviewListCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return cli.WithVault(func(v *vaultpkg.Vault) error {
				s := vaultpkg.NewVaultService(v, ops)
				entries, err := s.ListEntries("quarantine/")
				if err != nil {
	...
	importReviewPromoteCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return cli.WithVault(func(v *vaultpkg.Vault) error {
				s := vaultpkg.NewVaultService(v, ops)
				entries, err := s.ListEntries(quarantinePrefix)
				if err != nil {
				...
				for _, entryPath := range entries {
					...
					exists, existsErr := importEntryExists(s, destPath)
					if existsErr != nil {
					...
					// Read source entry
					entry, readErr := s.GetEntry(entryPath)
					if readErr != nil {
					...
					if writeErr := s.WriteEntry(destPath, entry); writeErr != nil {
						cli.PrintQuietAware("Warning: failed to write %s: %v\n", destPath, writeErr)
						hadError = true
						continue
					}
					if deleteErr := s.DeleteEntry(entryPath); deleteErr != nil {
...
func importEntryExists(s *vaultpkg.VaultService, entryPath string) (bool, error) {
	_, err := s.GetEntry(entryPath)
	if err == nil {
		return true, nil
	}
