...
def smoke(package: Path, arch: str) -> None:
    ...
    if arch not in GOARCH:
        raise ValueError(f"unsupported package target: {package_format}/{arch}")
    image, install = {
        "deb": ("ubuntu:24.04", "apt-get update; apt-get install -y --no-install-recommends git; dpkg -i /tmp/symvault.deb; dpkg -s symvault"),
        "rpm": ("fedora:44", "dnf install -y git /tmp/symvault.rpm; rpm -q symvault"),
        "apk": ("alpine:3.23", "apk add --allow-untrusted /tmp/symvault.apk git; apk info -e symvault"),
    }[package_format]
    binary = package.resolve()
    subprocess.run(
        [
            "docker", "run", "--rm", "--mount",
            f"type=bind,src={binary},dst=/tmp/symvault.{package_format},readonly", image,
            "sh", "-ec",
