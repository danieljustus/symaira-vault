...
func caseRemote() Case { root,e:=os.MkdirTemp("","sync-remote-");fail(e);defer os.RemoveAll(root); remote:=filepath.Join(root,"remote.git"); local:=filepath.Join(root,"local"); other:=filepath.Join(root,"other"); run(root,"git","init","--bare",remote); fail(git.Init(local)); fail(git.AddRemote(local,"origin",remote)); fail(os.WriteFile(filepath.Join(local,"entry.age"),[]byte("one"),0600)); fail(git.AutoCommit(local,"first")); push:=git.PushWithResult(local); run(local,"git","branch","--set-upstream-to=origin/master","master"); run(root,"git","clone",remote,other); fail(os.WriteFile(filepath.Join(other,"entry.age"),[]byte("two"),0600)); run(other,"git","config","user.name","Other");run(other,"git","config","user.email","other@example.com");run(other,"git","add","--all");run(other,"git","commit","-m","remote");run(other,"git","push","origin","HEAD"); pull:=git.PullWithResult(local); return Case{"GIT-002-local-bare","GIT-002",map[string]any{"remote":"origin","local_bytes":"one"},map[string]any{"push_success":push.Success,"push_skipped":push.Skipped,"push_has_remote":push.HasRemote,"pull_success":pull.Success,"pull_updated":pull.Updated,"pull_skipped":pull.Skipped,"pull_has_remote":pull.HasRemote,"pull_error":func() string { if pull.Error != nil { return pull.Error.Error() }; return "" }(),"remote_url_present":pull.RemoteURL!="","final_sha256":hash(mustRead(filepath.Join(local,"entry.age")))}} }
...
func main() {
	...
	f := Fixture{1, meta, cases, map[string]string{"native_watcher": "unproven; polling/process evidence is portable only", "pass_import": "unproven; production path requires external gpg", "archive_bytes": "not a byte contract; compare member manifest and restore result", "candidate_gaps": "GIT-002 divergent-pull parity and IO-003 native watcher remain unproven"}}
