...
func main() {
	...
	httpReq, err := http.NewRequest(req.Method, "http://"+listener.Addr().String()+req.Path, strings.NewReader(req.Body))
	check(err)
	httpReq.Header.Set("Content-Type", req.ContentType)
	httpReq.Header.Set("Accept", req.Accept)
	httpReq.Header.Set("MCP-Protocol-Version", req.ProtocolVersion)
	httpReq.Header.Set("Authorization", "Bearer "+token)
	httpReq.Header.Set("X-Symaira-Agent", "default")
	...
	for name := range httpResp.Header {
		if name == "Content-Type" {
			gotHeaders[name] = httpResp.Header.Get(name)
		}
	}
	gotHeaders["Content-Length"] = strconv.FormatInt(httpResp.ContentLength, 10)
	absent := []string{}
	...
	out := fixture{
		...
		ServerVersion: "1.0.0",
		Request:       req,
		Response:      response{Status: httpResp.StatusCode, Headers: gotHeaders, AbsentHeader: absent, Body: string(body)},
	}
