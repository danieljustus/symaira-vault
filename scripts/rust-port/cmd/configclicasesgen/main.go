...
func inputs() []inputCase {
	return []inputCase{
		...
		{
			name:        "get_nested_block_scalar_relative_indent",
			description: "get preserves intentional relative indentation inside a block scalar",
			config:      []byte("nested:\n  note: |\n    first\n      nested\n    last\n"),
			writeConfig: true,
			args:        []string{"config", "get", "nested", "--file", fileMarker},
		},
		{
			name:        "get_nested_block_scalar_chomping",
			description: "get preserves strip and keep chomping indicators in nested block scalars",
			config:      []byte("nested:\n  strip: |-\n    one\n      nested\n  keep: |+\n    two\n\n"),
			writeConfig: true,
			args:        []string{"config", "get", "nested", "--file", fileMarker},
		},
		...
		{
			name:               "set_literal_relative_indent",
			description:        "set preserves intentional relative indentation in a multiline string",
			config:             []byte("value: old\n"),
			writeConfig:        true,
			captureConfigAfter: true,
		...
		},
		{
			...
			description:        "set preserves a quoted string containing only a newline",
			config:             []byte("value: old\n"),
			writeConfig:        true,
			captureConfigAfter: true,
			args:               []string{"config", "set", "value", "\"\\n\"", "--file", fileMarker},
		...
		{
			name:               "set_newline_literal",
			description:        "set preserves a literal string containing only a newline",
			config:             []byte("value: old\n"),
			writeConfig:        true,
			captureConfigAfter: true,
			args:               []string{"config", "set", "value", "|\n  \n", "--file", fileMarker},
