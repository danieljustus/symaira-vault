...
func main() {
	...
	cases := []csvCase{
		...
		{Name: "bw_casefold_fields", Format: "bitwarden", Input: `{"Items":[{"Type":1,"Name":"x"}]}`},
		{Name: "bw_casefold_item_duplicate_variants_last_wins", Format: "bitwarden", Input: `{"items":[{"type":1,"name":"first","Name":"last"}]}`},
		{Name: "bw_casefold_duplicate_variants_last_wins", Format: "bitwarden", Input: `{"items":[{"type":1,"name":"first","Name":"last"}],"Items":[{"Type":1,"Name":"second"}]}`},
		{Name: "bw_casefold_unicode_simple_fold", Format: "bitwarden", Input: `{"itemſ":[{"Type":1,"Name":"folded"}]}`},
		{Name: "bw_casefold_nested_fields", Format: "bitwarden", Input: `{"FOLDERS":[{"ID":"f","NAME":"folder"}],"ITEMS":[{"Type":1,"Name":"x","FOLDERID":"f","LOGIN":{"USERNAME":"u","URIS":[{"URI":"https://example.test"}]}}]}`},
		{Name: "bw_casefold_duplicate_login_merges", Format: "bitwarden", Input: `{"items":[{"type":1,"name":"x","login":{"username":"a"},"LOGIN":{"password":"b"}}]}`},
		{Name: "bw_casefold_duplicate_login_null_preserves", Format: "bitwarden", Input: `{"items":[{"type":1,"name":"x","login":{"username":"a"},"LOGIN":null}]}`},
		{Name: "bw_casefold_duplicate_card_merges", Format: "bitwarden", Input: `{"items":[{"type":2,"name":"card","card":{"number":"123"},"CARD":{"code":"456"},"card":null}]}`},
		{Name: "bw_casefold_duplicate_scalar_null_preserves", Format: "bitwarden", Input: `{"items":[{"type":1,"Type":null,"name":"x","notes":"keep","NOTES":null}]}`},
		{Name: "bw_casefold_duplicate_slices_null_clears", Format: "bitwarden", Input: `{"items":[{"type":1,"name":"x","fields":[{"name":"custom","value":"keep"}],"FIELDS":null,"login":{"uris":[{"uri":"https://example.test"}]},"LOGIN":{"URIS":null}}]}`},
		...
		{Name: "bw_invalid_utf8_in_invalid_escape", Format: "bitwarden", InputB64: base64.StdEncoding.EncodeToString([]byte("{\"a\":\"é\",\"b\":\"\\\xff\"}"))},
		{Name: "bw_invalid_utf8_after_escaped_backslash", Format: "bitwarden", InputB64: base64.StdEncoding.EncodeToString([]byte("{\"items\":[{\"type\":1,\"name\":\"A\",\"login\":{\"password\":\"\\\\\xff\"}}]}"))},
		...
		{Name: "duplicate_header_casefold_last", Format: "csv", Input: "title,password,PASSWORD\nA,old,new\n"},
