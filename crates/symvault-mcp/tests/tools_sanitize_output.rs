...
fn runtime() -> Arc<ReadOnlyRuntime<EmptyStore>> {
    ...
    available_tools.push("sanitize_output".into());
    Arc::new(ReadOnlyRuntime::new(
        EmptyStore,
        ReadOnlyRuntimeConfig {
            server_name: "symvault".into(),
            server_version: "0.0.0-sanitize-output-fixture".into(),
...
#[test]
fn sanitize_output_matches_source_bound_go_fixture() {
    let fixture: Fixture = serde_json::from_str(include_str!(
        "../../../testdata/port/mcp/tools-sanitize-output.json"
    ))
    .expect("valid Go sanitize-output fixture");
    ...
    for case in fixture.cases {
        ...
        let actual = run_stream(&input, &mut handler)
            .expect("Rust sanitize-output stream dispatch")
            .into_iter()
