...
managed_hash: sha256:bb04b7694c3a6d8546d95f781238b0f2b52f183209e301c8cc3c867c6808de56
managed_installed_at: "2026-09-21T17:43:17Z"
managed_profile_tier: safe
---

# Symaira Vault (opencode)
...
First, call `symaira_whoami` to learn your permissions and which
vault you are connected to.

Vault path: `/var/folders/7v/7bpq0fjn44lflpxdylsx8qs40000gn/T/installcase-wrt2wbm0/vault`
Agent profile: **opencode** (tier: safe)

## Preferred Tools

Canonical Symaira Vault MCP tool names (opencode prefix: ``):

- `list_entries`
- `find_entries`
- `get_entry`
- `get_entry_metadata` — metadata only, no secrets
- `set_entry_field`
- `generate_password`
- `generate_totp`
- `delete_entry`
...
For `get_entry` and write access, ask the user to run:
  symaira agent upgrade opencode --tier standard
...
|---|---|---|
| `ERR_AUTH_FAILED` | Token expired or invalid | Call `symaira_whoami` |
...
to type the secret in chat. Call `request_credential` instead:
...
- Symaira Vault opens a native dialog. You receive only a confirmation, never the
  value itself.
- Continue the task using `execute_with_secret`,
  `copy_to_clipboard`, or `autotype` — never
...
## Agent-Specific: OpenCode

OpenCode uses `` as the tool prefix and `` for
slash commands.

To connect Symaira Vault to OpenCode:
```
symaira mcp-config opencode
```

Use OpenCode's native MCP configuration for tool discovery and management.
