...
managed_hash: sha256:69d1be2c288725e64482e6862a022191875ad03215fb93123342bec6cf6df536
managed_installed_at: "2026-09-21T17:43:17Z"
managed_profile_tier: safe
---

# Symaira Vault (hermes)
...
Vault path: `/var/folders/7v/7bpq0fjn44lflpxdylsx8qs40000gn/T/installcase-j28ljyv0/vault`
Agent profile: **hermes** (tier: safe)

## Preferred Tools

Canonical Symaira Vault MCP tool names (hermes prefix: `mcp_symaira_`):

- `mcp_symaira_list_entries`
- `mcp_symaira_find_entries`
- `mcp_symaira_get_entry`
- `mcp_symaira_get_entry_metadata` — metadata only, no secrets
- `mcp_symaira_set_entry_field`
- `mcp_symaira_generate_password`
- `mcp_symaira_generate_totp`
- `mcp_symaira_delete_entry`
- `mcp_symaira_copy_to_clipboard`
- `mcp_symaira_execute_with_secret`
- `mcp_symaira_request_credential`
...
For `get_entry` and write access, ask the user to run:
  symaira agent upgrade hermes --tier standard
...
|---|---|---|
| `ERR_AUTH_FAILED` | Token expired or invalid | Call `mcp_symaira_symaira_whoami` |
...
to type the secret in chat. Call `mcp_symaira_request_credential` instead:
...
- Symaira Vault opens a native dialog. You receive only a confirmation, never the
  value itself.
- Continue the task using `mcp_symaira_execute_with_secret`,
  `mcp_symaira_copy_to_clipboard`, or `mcp_symaira_autotype` — never
...
This agent supports slash commands via `/symaira:` prefix. Available
commands can be discovered through the agent's native command discovery.

## Agent-Specific: Hermes

Hermes uses the `mcp_symaira_` tool prefix and `/symaira:` slash prefix.

To verify MCP connectivity:
```
hermes mcp test symaira
```

Hermes supports scoped tokens via `symaira mcp token create --agent hermes`.
