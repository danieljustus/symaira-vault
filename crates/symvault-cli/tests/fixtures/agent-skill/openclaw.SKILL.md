...
Vault path: `{{VAULT}}`
Agent profile: **openclaw** (tier: safe)

## Preferred Tools

Canonical Symaira Vault MCP tool names (openclaw prefix: ``):
...
For `get_entry` and write access, ask the user to run:
  symaira agent upgrade openclaw --tier standard
...
## Agent-Specific: OpenClaw

OpenClaw uses `` as the tool prefix and `` for
slash commands.

OpenClaw supports native MCP tool discovery. Configure via the standard MCP
server setup in OpenClaw's configuration.
