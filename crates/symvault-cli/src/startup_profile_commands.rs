...
pub fn run(count: i64, _top: i64, trace: Option<&Path>) -> Result<(), String> {
    ...
    println!("Profiling startup time ({count} iterations)...\n");
    ...
    for iteration in 0..count {
        ...
        match child.status() {
            ...
            Ok(status) => {
                failures += 1;
                println!("  iteration {}: exec failed: {status}", iteration + 1);
            }
            Err(error) => {
                failures += 1;
                println!("  iteration {}: exec failed: {error}", iteration + 1);
            }
        }
    }
    ...
    println!("{}", "─".repeat(50));
    println!("  Min:        {}", go_duration(min));
    println!("  Max:        {}", go_duration(max));
    println!("  Avg:        {}", go_duration(average));
    println!("  P95:        {}", go_duration(p95));
    println!("  P99:        {}", go_duration(p99));
    println!("  Success:    {} / {count} iterations", times.len());
    if failures > 0 {
    ...
    println!("Environment");
    println!("{}", "─".repeat(50));
    println!("  Runtime:    Rust CLI");
    println!("  OS/Arch:    {}/{}", go_os(), go_arch());
    let cores = std::thread::available_parallelism()
        .map(|count| count.get())
        .unwrap_or(1);
    println!("  CPU:        {cores} core(s)");
    println!("  Parallelism: {cores}");
    println!();
    println!("Optimization Hints");
    println!("{}", "─".repeat(50));
    println!("  • Keep command startup paths free of vault and keychain work.");
    println!("  • Defer expensive service setup until a command needs it.");
    println!("  • Keep the default CLI dependency graph lean.");
    println!("  • Use an external profiler to inspect phase-level startup costs.");
    Ok(())
}
